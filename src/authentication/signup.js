...
import {
  askToConfirmOTP, registrationFailed, renderMainMenus, verificationFailed,
} from '../helpers/getSpecificMenu';
...
const registerUserMenus = async (textLength, text, phone) => {
  ...
  switch (true) {
    ...
    case (length === 2 && validateTextIsLettersOnly(lastName)):
      ...
      break;
    case (length === 3 && validateEmail(email)):
      message = `${strings.con.en} ${strings.country.en}`;
      break;
    case (length === 4 && validateTextIsLettersOnly(country)):
